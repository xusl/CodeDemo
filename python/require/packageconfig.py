...
g_modemimagePath = os.path.join(cwdname, 'ModemImage')
g_configxmlPath = os.path.join(cwdname, 'ModemImage', 'config.xml')
g_efsPath = os.path.join(cwdname, 'ModemImage', 'efs.mbn')
g_kernelADSUPath = os.path.join(cwdname, 'package', 'kernelADSU', 'ADSU.exe')
...
g_downloadimgTempPath = os.path.join(cwdname, 'package', 'Download.img')
g_winADSUPath = os.path.join(cwdname, 'ADSU', 'WIN')
g_winADSUfile = os.path.join(cwdname, 'ADSU','WIN', 'ADSU.exe')
g_tempADSUPath = os.path.join(cwdname, 'package', 'ADSU.exe')
g_signADSUPath = os.path.join(cwdname, 'package', 'signADSU.bat')
g_helpPath = os.path.join(cwdname, 'package', 'resource', 'Help.chm')
g_dynamicNVPath = os.path.join(cwdname, 'package', 'resource', 'dynamic_nv_281.xml')
g_iconPath = os.path.join(cwdname, 'package', 'resource', 'HspaUsbModem.ico')
g_strPath = os.path.join(cwdname, 'package', 'resource', 'string_multiLanguage.txt')
g_bmpPath = os.path.join(cwdname, 'package', 'resource', 'Upgrader.bmp')
g_PKINGPath = os.path.join(cwdname, 'package', 'PKIMG.exe')
nReleaseNoteTableItemCount = 13
docPath = os.path.join(cwdname, 'package', 'Release Notes.doc')
print docPath
g_customer_info_Path = os.path.join(cwdname, 'ModemImage', 'custom_info.xml')
Version = cwdname
...
class Project(object):
    ...
    def getPidVidText(self):
        ...
        if (project_code == "Y850") or (project_code == "L850") or \
           ...
           (project_code == "Y901"):
            return unicode('PID: PID_0195' + '\n' + 'VID: VID_1BBB')
        elif (project_code == "Y860") or (project_code == "Y859"):
            return unicode('PID: PID_0197' + '\n' + 'VID: VID_1BBB')
...
def modifyReleaseNote(docPath, PackageVersion, project):
    ...
    while nIndex < nReleaseNoteTableItemCount:
        ...
        if nameCellText.find(u"Package Version") == 0:
            ...
            row.Cells[1].Range.Text = unicode('External:' + external_ver + '\n' + 'Internal:' + internal_ver)
            ...
            row.Cells[1].Range.Text = project.getPidVidText()
        elif nameCellText.find(u"Simlock") == 0:
            if (PN_value == '0') and (PU_value == '0') and (PP_value == '0') and (PC_value == '0') and (PF_value == '0'):
                row.Cells[1].Range.Text = unicode('No simlock')
            elif (PN_value == '2') and (PU_value == '0') and (PP_value == '0') and (PC_value == '0') and (PF_value == '0'):
                row.Cells[1].Range.Text = unicode('Normal simlock')
            else:
                row.Cells[1].Range.Text = unicode('Has simlock')
...
def updateConfigIni(conf_ini = "config.ini", imgver=None):
    ...
    config['Packet']['ExeImgPath'] = g_tempADSUPath
    config['Packet']['SavePacketPath'] = g_downloadimgTempPath
    config['Packet']['HelpPath'] = g_helpPath
    config['Packet']['IconPath'] = g_iconPath
    config['Packet']['StringPath'] = g_strPath
    config['Packet']['PicturePath'] = g_bmpPath
    config['Packet']['DynamicNVPath'] = g_dynamicNVPath
    config['Packet']['ExePath'] = g_kernelADSUPath
    config['Packet']['SourceImagePath'] = g_modemimagePath
    config.write()
    '''
...
def main(winExec):
    ...
    if(os.path.exists(g_downloadimgPath)):
        if os.path.exists(g_downloadimgfile):
            ...
            print "Remove old one ok! Packing and move new Download.img OK!"
        else:
            shutil.move(g_downloadimgTempPath, g_downloadimgPath)
    ...
    if(os.path.exists(g_winADSUPath)):
        if os.path.exists(g_winADSUfile):
            ...
            print "Remove old one ok! Packing and move new ADSU.exe OK!"
        else:
            shutil.move(g_tempADSUPath, g_winADSUPath)
            print "Packing and move ADSU.exe OK!"
    else:
